export const cards = [
  { 
    title: 'Valorificare',
    frontImg:
      'https://i.shgcdn.com/545091da-9d3f-42e0-9bc4-a8d5a9f5b5b7/-/format/auto/-/preview/3000x3000/-/quality/lighter/',
    description:
      'Serviciile noastre de detailing ofer full aspect asupra tratamentelor pentru valorificarea, restaurare si viitoarele mentineri ale masinii',
    backImg: '',
    urlPath: '/valorificare'
  },
  {
    title: 'Protejare',
    frontImg:
      'https://i.shgcdn.com/ec2da442-5a27-440e-942e-f3a18059e2cb/-/format/auto/-/preview/3000x3000/-/quality/lighter/',
    description:
      'With our design paint protection film, we can cover any painted, carbon fibre or smooth finished paint of the car, even interior trims',
    backImg: '',
    urlPath: '/protejare'
  },
  {
    title: 'Mentinere',
    frontImg:
      'https://i.shgcdn.com/531fbc5c-8c09-45d1-8b44-9a716e120d1b/-/format/auto/-/preview/3000x3000/-/quality/lighter/',
    description:
      'Serviciile noastre de detailing ofer full aspect asupra tratamentelor pentru valorificarea, restaurare si viitoarele mentineri ale masinii',
    backImg: '',
    urlPath: '/mentinere'
  }
];

export const subcomponents = [
  {
    title: 'DESPRE',
    mainText:
      ' Detailing Auto Suceava sete cel mai exclusivist specialist în folii de protecție a vopselei și a detaliilor din Suceava, care se ocupă de cel mai mare volum de supermașini din întreaga judet. Facilitățile noastre de ultimă generație găzduiesc unii dintre cei mai calificați detalieri și instalatori de folie de protecție a vopselei din tara. Acest mediu, împreună cu expertiza noastră, ne permite să oferim clienților noștri cea mai înaltă calitate posibilă.',
    subText:
      'Noi va oferim un pachet total de soluții de îngrijire a mașinii, care încorporează totul, de la îmbunătățire și protecție până la întreținere',
    button: 'Vezi mai multe'
  },
  {
    title: 'INTREBARI & RASPUNSURI',
    mainText: 'Ai intrebari despre produsele noastre?',
    subText: 'Poti afla totul despre noi in sectiunea de mai jos',
    button: 'Vezi intrebarile',
    frontImg:
      'https://i.shgcdn.com/a856fb78-93ba-48de-94be-b251c59de508/-/format/auto/-/preview/3000x3000/-/quality/lighter/'
  }
];

export const valorificare = [
  {
    id: 1,
    title: 'Level 1 - Protejarea Detaliilor',
    subTitle: ' Exterior – Curatare/Clatire/Decontaminare.',
    header: '21 DE STAGII DESPRE PROCESUL DE CURATARE:',
    imgPath:
      'https://i.shgcdn.com/8152b67e-9de6-4ffc-a37c-25cb1217482c/-/format/auto/-/preview/3000x3000/-/quality/lighter/',

    description: [
      [
        'Vehiculul este spălat cu șampon cu PH neutru folosind un sistem de găleți de protecție dublă pentru a preveni transportarea pietrelor și a particulelor cu muchii ascuțite în mașină prin intermediul mănușii de spălat.'
      ],
      [
        'Roțile sunt curățate folosind un produs de curățare neacid pe bază de citrice și perii special concepute pentru păr de cal.'
      ],
      [
        'Închiderile ușilor, capacul benzinei, arcadele și părțile secțiunilor inferioare vor fi curățate (acolo unde sunt accesibile).'
      ],
      ['Vehiculul se usucă manual cu prosoape din microfibră adânci.'],
      [
        'Vopseaua este tratată cu o bară de argilă (ușoară sau agresivă) îndepărtând contaminanții de suprafață lipiți.'
      ],
      [
        'Solventul de corp este aplicat pentru a dizolva și îndepărta gudronul, murdăria de drum, depunerile de rouă de miere și stropi de insecte și alte substanțe pe bază de petrol.'
      ],
      ['Anvelopele sunt îmbrăcate.'],
      [' Materialele plastice exterioare și ornamentele ușilor sunt îmbrăcate.']
    ],
    sealent:
      'Vorbiți cu echipa noastră pentru recomandări privind acoperirile ceramice pentru vopsea, geamuri și jante. ',
    subDescription: {
      title: 'Interior',
      description: [
        ['Mocheta aspirata'],
        ['Geamurile sunt curatate pe interior'],
        ['Suprafețele și finisajele interioare sunt curățate cu produse de calitate aerospațială.']
      ]
    },

    whyChooseUS: [
      ['Suprafețele și finisajele interioare sunt curățate cu produse de calitate aero-spațială.'],
      [
      'Pentru a sigila vopseaua recent corectată, recomandăm un strat de ceramică pentru un plus de strălucire, proprietăți hidrofobe și întreținere ușoară sau înveliș protector de uretan termoplast pentru o protecție maximă.'
      ],
      [
      'Acesta este pachetul nostru de nivel inițial pentru corectarea vopselei care elimină imperfecțiunile minore, inclusiv zgârieturi fine în stratul superior de vopsea/vernish. Este perfect pentru mașinile noi sau cele cu vopsea estompată care au nevoie de revigorare. Acest proces rezultă într-un „factor de corecție” de 70% - 80%, nu va înlătura însă nicio zgârietură mai adâncă.'
      ]
      ]
  },
  {
    id: 2,
    title: 'Level 2 - Detaliu de îmbunătățire',
    subTitle: ' Exterior – Curatare/Clatire/Decontaminare.',
    header: 'PROCESUL DE CURĂȚARE Detailing Auto Suceava ÎN MAI MULTE ETAPE ETAPE, INCLUSIV:',
    imgPath:
      'https://i.shgcdn.com/297bf929-a9fe-4ae1-afdc-acb9b92042f7/-/format/auto/-/preview/3000x3000/-/quality/lighter/',
    description: [
      [
        'Un strat de spumă albă groasă este aplicat pe vehicul pentru a ajuta la slăbirea, ridicarea și îndepărtarea particulelor de murdărie de pe vopsea fără a induce micro zgârieturi.'
      ],
      [
        'Vehiculul este apoi uscat cu un debit maxim de aer „Black Baron Vehicle Dryer” de puțin peste 330 mph, eliminând urmele de apă.'
      ],
      ['Carcasa motorului este curățată.'],
      ['Vehiculul se usucă manual cu prosoape din microfibră adânci.']
    ],

    subDescription: {
      title: 'Interior',
      description: [
        [
          'Vehiculul este dezodorizat folosind un tratament anti-bacterian special conceput, care se descompune și elimină mirosurile neplăcute și bacteriile in loc sa le mascheze'
        ],
        ['Geamurile sunt curatate pe interior'],
        ['Interior trim and surfaces are all cleaned using aerospace-grade products.']
      ]
    },

    whyChooseUS: [
      ['De ce să alegeți Detaliul de îmbunatatire de nivel 2?'],
      [
        'Pentru a etanșa vopseaua proaspăt corectată, vă recomandăm acoperire ceramica pentru un plus de luciu, proprietăți hidrofobe și întreținere mai ușoară sau PPF pentru o protecție maximă.'
      ],
      [
        'Acesta este detaliul nostru de corectare a vopselei de nivel de intrare, care îndepărtează imperfecțiunile minore, inclusiv vârtejurile/zgârieturile fine din stratul transparent/lacul superior al vopselei. Este perfect pentru mașini noi sau mașini cu vopsea tocită care au nevoie de o reîmprospătare. Acest proces are ca rezultat un „factor de corecție” de 70% până la 80%, însă nu va elimina zgârieturile mai adânci.'
      ]
    ]
  },
  {
    id: 3,
    title: 'Level 3 - Corectarea Detaliilor',
    subTitle: ' CORECTAREA VOPSEA – 80-95% FACTOR DE CORECTARE',
    imgPath: "https://i.shgcdn.com/f432b83b-7fb0-4201-8036-08b66e7933c0/-/format/auto/-/preview/3000x3000/-/quality/lighter/",
    description: [
      [
        'Grosimea vopselei este măsurată cu ajutorul unui indicator de adâncime a vopselei cu ultrasunete.'
      ],
      ['Pentru stabilirea stării vopselei se utilizează diverse lumini.'],
      [
        'Vopseaua este tratată printr-un proces de corectare a vopselei în trei etape pentru a elimina defectele, inclusiv urme medii de turbionare și holograme. Panourile sunt șters cu alcool după fiecare etapă pentru a asigura corectarea adevărată. Vârfurile de evacuare sunt întinerite cu compuși metalici special proiectați. Sticla este tratată cu nano polish'
      ],
      ['Vehiculul se usucă manual cu prosoape din microfibră adânci.']
    ],

    subDescription: {
      title: 'Interior',
      description: [
        [
          'Covoarele sunt șamponate dacă este necesar și uscate folosind un aspirator umed cu aspirație. Detergentul special conceput pentru Alcantara este aplicat pentru a revigora fibrele de Alcantara. Toate suprafețele din piele sunt curățate și protejate. Orificiile de ventilație, împrejurimile butoanelor și fascia sunt curatate cu perii specializate'
        ],
        ['Geamurile sunt curatate pe interior'],
        ['Interior trim and surfaces are all cleaned using aerospace-grade products.']
      ]
    },

    whyChooseUS: [
      ['De ce să alegeți Detaliul de corectare de level 3?'],
      [
        'Acesta este vehiculul nostru uzat pentru corectarea vopselei în trei etape și detaliile interioare, care îndepărtează imperfecțiunile medii, inclusiv vârtejurile/zgârieturile din stratul transparent al vopselei. Acest proces are ca rezultat 80% până la 95% „factor de corecție” '
      ],
      [
        'Detaliul de nivel 3 include, de asemenea, o întinerire completă a tuturor elementelor cromatice și metalice, de ex. vârfurile de evacuare, închiderea ușilor, grupurile de lumină și aer, spălarea interiorului și dezodorizarea, protecția pielii, lustruirea nano a sticlei în interior și în exterior și multe altele.'
      ],
      [
        'Acest proces este conceput pentru a scoate la iveală tot ce este mai bun dintr-o mașină uzată, reluând viața vopselei și arătând cât se poate de nou! '
      ],
      ['Acest nivel de detaliere nu înlătură nicio zgârietură care a trecut prin vopsea.'],
      [
        'Pentru a etanșa vopseaua proaspăt corectată, vă recomandăm Ceramic Coating pentru un plus de luciu, proprietăți hidrofobe și întreținere mai ușoară sau PPF pentru o protecție maximă.'
      ]
    ]
  },

  {
    id: 4,
    title: 'Level 4 - Detaliu suprem',
    subTitle: ' Exterior – Curatare/Clatire/Decontaminare.',
    // header: '21 stagii DESPRE PROCESUL DE CURATARE:',
    imgPath:
      'https://i.shgcdn.com/f84730ff-b168-4edf-bf06-55cb317626d5/-/format/auto/-/preview/3000x3000/-/quality/lighter/',
    description: [
      ['Închiderile ușilor sunt curățate.'],
      [
        'Roțile sunt îndepărtate, curățate pe interior și curățate, apoi toate piulițele roților sunt reglate cuplul conform specificațiilor corecte ale producatorului.'
      ],
      ['Roțile sunt curățate, lustruite și sigilate în interior și în exterior.'],
      ['compartimentul motorului este detaliat.'],
      [
        'Vopseaua este tratată printr-un proces de corectare a vopselei în trei până la cinci etape pentru a elimina defecte, cum ar fi urme puternice de vârtej, holograme și zgârieturi minore.'
      ],
      [
        'Zgârieturile mai adânci ale suprafeței sunt nivelate folosind hârtie de finisare special concepută. Vopseaua este verificată folosind un microscop digital în anumite zone.'
      ]
    ],
    sealent: 'CORECTAREA VOPSEA – 95%-99% FACTOR DE CORECTARE',
    subDescription: {
      title: 'Interior',
      description: [
        ['Centurile de siguranță sunt curatate cu abur'],
        [
          'Toate suprafețele din piele sunt curățate, de-lucidate, alimentate cu uleiuri esențiale și apoi protejate cu protecție din piele.'
        ]
      ]
    },

    whyChooseUS: [
      ['De ce să alegeți Nivelul 4 de Detailing Suprem?'],
      [
        'Acest nivel de detaliere este ideal dacă doriți să readuceți la viață vopseaua mașinii. În plus față de ceea ce oferim în nivelurile noastre inferioare de detaliere, nivelul 4 de detaliu include un detaliu al compartimentului motorului, curățarea sub caroserie, roțile îndepărtate și lustruite și multe altele. Cu până la cinci etape de corecție putem obține un „factor de corecție” de până la 99%. Acesta este cel mai bun detaliu posibil pentru a întineri vopseaua și a crea un finisaj „mai bun decât nou”.'
      ],
      [
        'Pentru a etanșa vopseaua proaspăt corectată, vă recomandăm Ceramic Coating pentru un plus de luciu, proprietăți hidrofobe și întreținere mai ușoară sau PPF pentru o protecție maximă..'
      ]
    ]
  },
  {
    id: 5,
    title: 'Level 5 - Detaliu semnătură',
    subTitle: ' Exterior – Curatare/Clatire/Decontaminare.',
    header: 'Indepartarea distorsiunii vopselei PANA LA 100% FACTOR DE CORECTARE',
    imgPath:
      'https://i.shgcdn.com/509fdfbc-571b-4fc3-8ed7-efcc314d7ee4/-/format/auto/-/preview/3000x3000/-/quality/lighter/',
    description: [
      [
        'Accentul acestui serviciu este de a elimina finisajul inconsecvent și distorsionat găsit în straturile transparente moderne. Tratamentul nostru Signature se concentrează pe îmbunătățirea finisajului din fabrică, obținând un standard mult superior.'
      ],
      [
        'Procesul include nivelarea stratului transparent (lacul) folosind o varietate de instrumente specializate și eliminarea punctelor importante întâlnite pe sistemele moderne de vopsea.'
      ],
      [
        'Expertiza noastră, cunoștințele și anii de experiență se combină pentru a oferi tratamentului D&S, rata de succes de 100%'
      ]
    ],
    sealent: '',
    subDescription: {
      title: 'De ce sa alegeti Detaliul de Nivel 5?',
      description: [
        [
          'Dacă vă străduiți să obțineți perfecțiunea vopselei dvs., atunci Detaliul nostru de semnătură este pentru dvs. „Coaja de portocală” sau vârfurile și jgheaburile din stratul transparent adaugă vopselei nedorite, distorsiuni, textură și mate. Detaliul D&S va elimina textura nedorită de „coajă de portocală”, obținând un finisaj asemănător sticlei.'
        ]
      ]
    },

    whyChooseUS: [
      [
        'Detaliul nostru de nivel 5 este Detaliul nostru de semnătură, care realizează complet stratul transparent, rafinându-l și lustruindu-l în cinci etape. Aceasta implică șlefuirea uscată, șlefuirea umedă și corectarea vopselei. Acest tratament este perfect pentru o mașină pe care doriți să o păstrați și să o prețuiți mulți ani.'
      ],
      [
        'Pentru a etanșa vopseaua proaspăt corectată, vă recomandăm Ceramic Coating pentru un plus de luciu, proprietăți hidrofobe și întreținere mai ușoară sau PPF pentru o protecție maximă.'
      ]
    ]
  },
  {
    id: 6,
    title: 'Level 6 - Protejarea Detaliilor',
    subTitle: ' Exterior – Curatare/Clatire/Decontaminare.',
    header: '21 stagii DESPRE PROCESUL DE CURATARE:',
    imgPath:
      'https://i.shgcdn.com/297bf929-a9fe-4ae1-afdc-acb9b92042f7/-/format/auto/-/preview/3000x3000/-/quality/lighter/',
    description: [
      [
        'Vehiculul este spălat cu șampon cu PH neutru folosind un sistem de găleți de protecție dublă pentru a preveni transportarea pietrelor și a particulelor cu muchii ascuțite în mașină prin intermediul mănușii de spălat.'
      ],
      [
        'Roțile sunt curățate folosind un produs de curățare neacid pe bază de citrice și perii special concepute pentru păr de cal.'
      ],
      [
        'Închiderile ușilor, capacul benzinei, arcadele și părțile secțiunilor inferioare vor fi curățate (acolo unde sunt accesibile).'
      ],
      ['Vehiculul se usucă manual cu prosoape din microfibră adânci.'],
      [
        'Vopseaua este tratată cu o bară de argilă (ușoară sau agresivă) îndepărtând contaminanții de suprafață lipiți.'
      ],
      [
        'Solventul de corp este aplicat pentru a dizolva și îndepărta gudronul, murdăria de drum, depunerile de rouă de miere și stropi de insecte și alte substanțe pe bază de petrol.'
      ],
      ['Anvelopele sunt îmbrăcate.'],
      [' Materialele plastice exterioare și ornamentele ușilor sunt îmbrăcate.']
    ],
    sealent:
      '"Vorbiti cu echipa noastra pentru recomandari privind acoperirile ceramice pentru vopsea, geamuri si roti." ',
    subDescription: {
      title: 'Interior',
      description: [
        ['Mocheta aspirata'],
        ['Geamurile sunt curatate pe interior'],
        ['Interior trim and surfaces are all cleaned using aerospace-grade products.']
      ]
    },

    whyChooseUS: [
      ['Suprafețele interioare și finisajele sunt curățate cu produse de calitate aeronautică.'],
      [
      'Pentru a sigila suprafețele vopsite proaspăt corectate, recomandăm un strat de acoperire ceramic pentru proprietățile sale hidrofobe, strălucirea și ușurința în întreținere sau PPF pentru protecție maximă.'
      ],
      [
      'Acesta este serviciul nostru de corecție a vopselei de nivel de bază, care îndepărtează imperfecțiunile minore, inclusiv urme fine/ zgârieturi în stratul exterior al vopselei. Este perfect pentru mașinile noi sau cele cu vopsea ștearsă care au nevoie de o revigorare. Acest proces rezultă într-un factor de corecție de 70-80%, nu va elimina însă zgârieturi adânci mai mari.'
      ]
      ]
  }
];
